...
fn temp_dir(name: &str) -> PathBuf {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let path = std::env::temp_dir().join(format!("logcut-test-{name}-{}-{unique}", std::process::id()));
...
fn success_suppresses_output_and_removes_log() {
    ...
    assert_eq!(fs::metadata(&logs).unwrap().permissions().mode() & 0o777, 0o700);
...
fn stdin_is_forwarded_without_success_output_leaking() {
    ...
    let mut child = Command::new(binary())
        .env("LOGCUT_LOG_DIRECTORY", &logs)
        .args(["sh", "-c", "cat >\"$1\"", "_", destination.to_str().unwrap()])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
fn all_profiles_extract_expected_failure() {
    ...
    for (profile, body, expected) in fixtures {
        ...
        let output = run(profile, Some(profile), &format!("printf \"{escaped}\"; exit 1"));
        let text = combined(&output);
...
fn playwright_profile_and_auto_detection_are_concise() {
    ...
    for profile in [Some("playwright"), None] {
        let output = run("playwright", profile, &format!("printf \"{escaped}\"; exit 1"));
        let text = combined(&output);
